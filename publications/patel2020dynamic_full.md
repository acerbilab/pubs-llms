...
where $\zeta_{s a} \stackrel{\text { i.i.d. }}{\sim} \mathcal{N}(0,1) \forall s \in \mathcal{S}, a \in \mathcal{A}$, and $\beta$ is the inverse-temperature parameter for softmax. Plugging this into Eq. 3 for each step in the sampled trajectories yields the stochastic gradient of the first term of $\mathcal{F}$, the expected reward, with respect to $\sigma$.
...
Algorithm 1: Dynamic Resource Allocator (DRA)
    ...
    Initialize \(\overline{\boldsymbol{q}}, \boldsymbol{\sigma}\), table of memories \(=\left\langle s, a, r, s^{\prime}, \mathcal{N}\left(\bar{q}_{s a}, \sigma_{s a}^{2} I\right)\right\rangle^{|\mathcal{S} \times \mathcal{A}|}\)
    ...
    Sample \(N\) trajectories to compute:
        \(\nabla_{\sigma} \mathbb{E}_{\pi}\left[\sum_{t=0}^{\infty} \gamma^{t} r_{t+1} \mid Q\right]=\frac{1}{N} \sum_{n=1}^{N}\left[\sum_{t=0}^{\infty} \Psi_{t} \nabla_{\sigma} \log \pi\left(a_{t} \mid s_{t}\right)\right]\)
                                    where \(\Psi_{t} \leftarrow\) Eq. 4
                                    \(\nabla_{\sigma} \log \pi\left(a_{t} \mid s_{t}\right) \leftarrow\) Eq. 5
...
$$
\widetilde{\mathcal{F}}:=\frac{\mathbb{E}_{\pi}\left[\max _{\text {path }=1}^{b M}\left(\sum_{t=1}^{M} r_{t, \text { path }}\right) \mid Q\right]}{\left\langle a_{\text {dec }} b+t_{\text {non-dec }}\right\rangle}-\lambda D_{\mathrm{KL}}(Q \| P)
$$
